             ...
             Dictionary<string, string> result)
        {
            ...
            for (int i = 0; i < text.Count - 1; i++)
            {
                ...
                for (int j = 0; j < sentence.Count - 1; j++)
                {
                    ...
                    {
                        if (string.CompareOrdinal(nextWord, result[word]) < 0) // -> check what word is less
                            if (IsMaxFrequency(frequency, word, nextWord)) // -> check if the word is more frequently than all others
